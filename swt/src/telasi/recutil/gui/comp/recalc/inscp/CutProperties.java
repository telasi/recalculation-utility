...
public class CutProperties extends Dialog {
	...
	public CutProperties(Shell parent, int style) {
		super(parent, style);
	}
			...
			dialogShell.layout();
			{
				...
				composite1.setLayout(composite1Layout);
				{
					...
					lblTitle.setText(GUIMessages
							.getMessage("comp.cut_properties.title"));
			...
			}
			{
				...
				}
				{
					...
					lblDescription.setText(GUIMessages
							.getMessage("comp.cut_properties.descr"));
				}
			}
			{
				...
				composite3.setLayout(composite3Layout);
				{
					...
					lblSubtitle1.setText(GUIMessages
							.getMessage("comp.cut_properties.subtitle1"));
	...
	private void reset() {
		if (item == null && recalculation != null) {
			...
			dialogShell.setText(GUIMessages
					.getMessage("comp.cut_properties.new_title"));
		} else if (item != null) {
			pkStartDate.setDate(item.getStartDate());
			pkEndDate.setDate(item.getEndDate());
			isNewMode = false;
			dialogShell.setText(GUIMessages
					.getMessage("comp.cut_properties.edit_title"));
		} else {
			pkStartDate.setDate(null);
			pkEndDate.setDate(null);
	...
	private void onOk() {
		...
		RecalcCutItem newItem = new RecalcCutItem();
		...
		try {
			if (isNewMode) {
				RecalcCutInsertRequest request = new RecalcCutInsertRequest(
				...
				RecalcCutInsertRequest callback = (RecalcCutInsertRequest) resp
						.getRequest();
				...
				RecalcCutUpdateRequest request = new RecalcCutUpdateRequest(
						Application.USER_NAME, Application.PASSWORD);
				request.setItem(newItem);
